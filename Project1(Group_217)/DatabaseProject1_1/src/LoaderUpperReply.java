...
public class LoaderUpperReply {
    ...
    public static void setPrepareStatement() {
        try {
            stmt = con.prepareStatement("INSERT IGNORE INTO public.Authors (movieid, title, country, year_released, runtime) " +
                    "VALUES (?,?,?,?,?);");
        } catch (SQLException e) {
    ...
    public static void main(String[] args) {
        ...
        try {
            stmt = con.prepareStatement("INSERT INTO public.upper_replies (reply_id, upper_reply_id) " +
                    "VALUES (?,?)" + " ON CONFLICT (reply_id, upper_reply_id) DO NOTHING;");
        } catch (SQLException e) {
            System.err.println("Insert statement failed");
            System.err.println(e.getMessage());
            closeDB();
        ...
        for (SingleReply r: list) {
            if (r.getUpper_floor1() == null) {
                continue;
            }
            if (con != null) {
                try {
                    stmt.setInt(1, r.getReply_id1());
                    stmt.setInt(2, r.getUpper_floor1());
                    stmt.execute();
                    cnt++;
                    if (cnt % 1000 == 0) {
